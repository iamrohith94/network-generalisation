...
query = "SELECT source,target,COUNT(*)  FROM paths where actual_distance > 4000 GROUP BY source,target limit 30"
cur.execute(query)
rows = cur.fetchall()
for row in rows:
	level = []
	distance = []
	query = "SELECT contracted_distance, level FROM paths where source = %s and target = %s ORDER BY level DESC"
