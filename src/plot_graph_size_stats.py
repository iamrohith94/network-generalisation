...
def get_average_percent(averages, threshold, level): 
	...
	plt.xlabel('Percentage of Graph Size used')
	plt.ylabel('Average Percentage Difference in distance')
	#plt.title('Threshold: '+str(threshold))
	plt.tick_params(axis='both', which='major', labelsize=7)
	plt.savefig('../images/'+db+'_graph_size_stats_'+str(level)+'.png',facecolor='white')
	plt.clf()
...
for level in levels:
	...
	query = "SELECT ((contracted_distance - actual_distance)*100.00)/actual_distance, (contracted_graph_edges*100.00)/original_graph_edges FROM paths WHERE level = %s AND actual_distance >= %s"
	cur.execute(query, (level, threshold, ))
	rows = cur.fetchall()
	dist_diff = []
	graph_percent = []
	for row in rows:
